...
double SNPHWE(int obs_hets, int obs_hom1, int obs_hom2) {
   ...
   Rcpp::NumericVector het_probs(rare_copies + 1);

   /* start at midpoint */
   int mid = rare_copies * (2 * genotypes - rare_copies) / (2 * genotypes);
   /* check to ensure that midpoint and rare alleles have same parity */
   if ((rare_copies % 2) != (mid % 2)) mid++;
   // if (obs_hets == mid) return 1.0;
   ...
   for (curr_hets = mid; curr_hets > 1; curr_hets -= 2) {
      ...
      double prob = het_probs[curr_hets] * curr_hets * (curr_hets - 1.0) /
         (4.0 * (curr_homr + 1.0) * (curr_homc + 1.0));
      if (prob == 0) break;

      het_probs[curr_hets - 2] = prob;
      sum += prob;
   ...
   for (curr_hets = mid; curr_hets <= rare_copies - 2; curr_hets += 2) {
      ...
      double prob = het_probs[curr_hets] * 4.0 * curr_homr * curr_homc /
   ...
   double max_het = curr_hets;

   double p_hwe = 0.0;
   double thr = het_probs[obs_hets];
   // Rcpp::Rcout << het_probs[obs_hets] << std::endl;
   /*  p-value calculation for p_hwe  */
   for (int i = min_het; i <= mid; i += 2) {
      if (het_probs[i] > thr) {
         break;
      } else {
         p_hwe += het_probs[i];
...
// [[Rcpp::export]]
double SNPHWE_origin(int obs_hets, int obs_hom1, int obs_hom2)
{
