...
system.time(ind.keep <- bed_clumping(obj.bed, ncores = nb_cores(),
                                     exclude = snp_indLRLDR(CHR, POS))) # 55-60 sec
...
system.time(svd3 <- bed_autoSVD(obj.bed, ncores = nb_cores(), thr.r2 = 0.2))
# 2091 sec -> 35 min
attr(svd3, "lrldr")
...
gwas <- bed_pcadapt(obj.bed, U.row = svd3$u, ncores = nb_cores())
...
system.time(
  svd4 <- bed_randomSVD(obj.bed, ind.col = ind.keep2, ncores = nb_cores())
)

gwas2 <- bed_pcadapt(obj.bed, U.row = svd4$u, ncores = nb_cores())
...
system.time(
  svd5 <- bed_autoSVD(obj.bed, ind.row = ind.row, ind.col = ind.keep2, ncores = nb_cores())
)
...
system.time(
  svd6 <- bed_autoSVD(obj.bed, ind.row = ind.row2,
