...
glasso <- runonce::save_run({
  S <- crossprod(scale(G[], center = TRUE, scale = FALSE)) / (nrow(G) - 1)
  glassoFast::glassoFast(S, rho = 0.01, trace = TRUE)
}, file = "tmp-data/glasso_chr22.rds")
mean(glasso$wi == 0) # 99%

diag(glasso$w)
inv <- solve(glasso$wi)
all.equal(inv, glasso$w) # diff: 0.002230628
prec <- as(glasso$wi * tcrossprod(sqrt(diag(inv))), "dsCMatrix")
object.size(corr) / object.size(prec)  # 12.8
inv2 <- Matrix::solve(prec)
print(range(diag(inv2)), digits = 20) # 0.99999999999983524 1.00000000000055445
