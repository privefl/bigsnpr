...
p <- bigstatsr::big_colstats(X)$sum / (2 * nrow(X))
maf <- pmin(p, 1 - p)
print(system.time(
  ind.keep <- snp_clumping(celiac, maf, thr.corr = 0.1,
                             exclude = which(maf < 0.05),
                             ncores = 3)
))
# 65,610 SNPs & 3.5 min
# 62,938 SNPs & 3.2 min

X2 <- deepcopy(X, cols = ind.keep, shared = FALSE)
print(system.time(
  pca <- bigstatsr::big_SVD(X2, fun.scaling = snp_scaleBinom, k = 10)
))
