...
This tutorial uses fake data for educational purposes only. It would be nice to use some better fake data for this tutorial; any idea?
Another tutorial using more realistic data can be found at https://privefl.github.io/bigsnpr-extdoc/polygenic-scores-pgs.html.
...
Here, we have built the LD matrix using variants from one chromosome only. In practice, you need to build it for variants from all chromosomes. Please look at the code from the paper or the other tutorial (links at the beginning).
...
```{r}
(h2_seq <- round(h2_est * c(0.1, 0.7, 1, 1.4), 4))
...
Note that missing values represent models that diverged substantially. Here the data is very small and very fake, so there are some problems with the Gibbs sampler.
...
You can then choose the best model according to your preferred criterion (e.g. max AUC). Here, we use the Z-Score from the (logistic) regression of the phenotype by the PRS since we have found it more robust than using the AUC. It also enables adjusting for covariates in this step.
...
pred <- big_prodVec(G, best_grid, ind.row = ind.test, ind.col = ind.chr2)
...
One can also check the estimate of the correlations (between PGS and phenotype) compared to the actual value (from the summary statistics):

```{r}
corr_hat <- with(df_beta, beta / sqrt(n_eff * beta_se^2 + beta^2))
drop(cor(sapply(multi_auto, function(auto) auto$corr_est), corr_hat))
```
